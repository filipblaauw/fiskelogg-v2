                    ...
                    <form class="form-horizontal form-material" id="loginform" @submit.prevent="submit" @keydown="loginForm.errors.clear($event.target.name)">
                        ...
                        <div class="row" v-if="getConfig('social_login')">
                            <div class="col-xs-12 col-sm-12 col-md-12 m-t-10 text-center">
                                <div class="social">
                                    <a v-for="provider in social_login_providers" v-if="getConfig(provider+'_login')" :href="`/auth/social/${provider}`" class="m-r-5" v-tooltip="trans('auth.login_with',{type:provider})"> <i :class="['fab','fa-'+provider, 'fa-2x']"></i></a>
                                </div>
                            </div>
                        </div>
                        <div class="row m-t-10" v-if="!getConfig('mode')">
                            <div class="col-6 text-center">
                                <button class="btn bt-block btn-info" @click="loginAsAdmin">Login as Admin</button>
                            </div>
                            <div class="col-6 text-center">
                                <button class="btn bt-block btn-success" @click="loginAsUser">Login as User</button>
                            </div>
                        </div>
                    </form>
                </div>
